const state = {
  ...
  lineupSlots: [],
  selectedSlot: null,
  pickerMode: "starter",constconst state = {
  db: null,
  level: "Sub13",
  dataLevel: "Sub13",
  selectedMatch: null,
...
function renderDataPage() {
  ...
  const summary = levelSummary(level);
  $("#dataCards").innerHTML = [
    ["Jogos", summary.matches, "", "J"],
    ["Realizados", summary.finished, "", "R"],
    ["Por jogar", summary.scheduled, "", "P"],
    ["Vitórias", summary.wins, "result-win", "V"],
    ["Empates", summary.draws, "result-draw", "E"],
    ["Derrotas", summary.losses, "result-loss", "D"],
    ["Golos marcados", summary.goalsFor, "goals-for", "GM"],
    ["Golos sofridos", summary.goalsAgainst, "goals-against", "GS"],
  ]
    .map(([label, value, tone, icon]) => `<article class="stat-card ${tone}"><i>${icon}</i><span>${label}</span><strong>${value}</strong></article>`)
    .join("");
